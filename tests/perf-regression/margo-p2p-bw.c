...
int main(int argc, char **argv) 
{
    ...
    if(self == 1)
    {
        ...
        else
        {
            ...
            assert(bw_worker_scheds);

            ret = ABT_pool_create_basic(ABT_POOL_BLOCKING_FIFO, ABT_POOL_ACCESS_MPMC, ABT_TRUE, &g_transfer_pool);
            assert(ret == ABT_SUCCESS);
            for(i=0; i<g_opts.threads; i++) 
            {
                ret = ABT_sched_create_basic(ABT_SCHED_BASIC, 1, &g_transfer_pool,
